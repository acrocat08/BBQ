...
namespace BBQ.Cooking {
    ...
    public class MissionSheetView : ScriptableObject {
        ...
        public void Init(MissionSheet missionSheet, List<MissionStatus> missions) {
            Transform container = missionSheet.transform.Find("Container");
            if (missions.Count == 0) return;
            UpdateText(container, missions[0], false);
        ...
        public void UpdateText(Transform mission, MissionStatus status, bool showFailIcon) {
            ...
            Text count = mission.Find("Num").GetComponent<Text>();
            Text nowCount = mission.Find("NowNum").GetComponent<Text>();
            //string baseDetail = status.mission.detail;
            //string[] split = baseDetail.Split("#");
            //detail.text = "-　" + split[0] + status.goal + split[1];
            count.text = status.goal.ToString();
            nowCount.text = status.now.ToString();
            mission.Find("OK").GetComponent<Image>().enabled = status.now >= status.goal;
